...
class DetectText extends StatefulWidget {
  @override
  _DetectTextState createState() => _DetectTextState();
}
...
class _DetectTextState extends State<DetectText> {
  ...
  detectObject(File image) async {
    Map<String, dynamic> results = await _callapi.getresults(image, "text");
    FileImage(image)
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      texttospeak = texttospeak + " '${results[index]['text']}' ";
    }
    _tts.tts(
  ...
  List<Widget> renderBoxes(Size screen) {
    ...
    for (var i = 0; i < _recognitions.length; i++) {
      ...
      double left = double.parse(_recognitions[index]["left"]);
      double top = double.parse(_recognitions[index]["top"]);
      double width = double.parse(_recognitions[index]["W"]);
      double height = double.parse(_recognitions[index]["H"]);
        ...
        child: Positioned(
            ...
            child: ((double.parse(_recognitions[index]["confidence_score"]) >
                ...
                ? Container(
                    ...
                    child: Text(
                      "${_recognitions[index]["text"]} ${(double.parse(_recognitions[index]["confidence_score"]) * 100).toStringAsFixed(0)}%",
                      style: TextStyle(
